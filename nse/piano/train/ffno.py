...
class SpectralConv2d_dy(nn.Module):
    def __init__(self, K, in_dim, out_dim, n_modes, forecast_ff, backcast_ff,
                 fourier_weight, factor, ff_weight_norm,
                 n_ff_layers, layer_norm, use_fork, dropout, mode):
        super().__init__()
        self.K = K
...
class FNOFactorized2DBlock(nn.Module):
    def __init__(self, d_embds=256, K=4, T=10, modes=12, width=60, input_dim=20, dropout=0.0, in_dropout=0.0,
                 n_layers=6, share_weight: bool = False,
        ...
        self.spectral_layers_dy = SpectralConv2d_dy(K, in_dim=width,
                                                    out_dim=width,
                                                   n_modes=modes,
        ...
        self.out = nn.Sequential(
            ...
            WNLinear(128, 20, wnorm=ff_weight_norm))

    def forward(self, x, embedding, **kwargs):
        ...
        for i in range(self.n_layers):
            if i==0:
                layer = self.spectral_layers_dy
                b, f = layer(x, att)
            else:
                layer = self.spectral_layers[i-1]
                b, f = layer(x)
