...
def main(cfg):
    ...
    sys.stdout.flush()
    encoder = FNO_pretrain(cfg.modes, cfg.tw_in, cfg.width, cfg.h_size)
    ...
    train_data = torch.load(data_path+'data_train').to(device)[..., :20].float()
    test_data = torch.load(data_path+'data_test').to(device)[..., :20].float()
    val_data = torch.load(data_path+'data_val').to(device)[..., :20].float()

    train_data_a = train_data[:, :32, :32, :]
    train_data_b = train_data[:, 32:, :32, :]
    train_data_c = train_data[:, :32, 32:, :]
    train_data_d = train_data[:, 32:, 32:, :]

    test_data_a = test_data[:, :32, :32, :]
    test_data_b = test_data[:, 32:, :32, :]
    test_data_c = test_data[:, :32, 32:, :]
    test_data_d = test_data[:, 32:, 32:, :]

    val_data_a = val_data[:, :32, :32, :]
    val_data_b = val_data[:, 32:, :32, :]
    val_data_c = val_data[:, :32, 32:, :]
    val_data_d = val_data[:, 32:, 32:, :]

    train_embedding_a = net.encoder(train_data_a)
    train_embedding_b = net.encoder(train_data_b)
    train_embedding_c = net.encoder(train_data_c)
    train_embedding_d = net.encoder(train_data_d)
    train_embedding = torch.concat([train_embedding_a, train_embedding_b, train_embedding_c, train_embedding_d], dim=-1)
    train_embedding = train_embedding/torch.norm(train_embedding, dim=1)[..., None]
    torch.save(train_embedding, f'embedding/train_embedding_{seed}')

    test_embedding_a = net.encoder(test_data_a)
    test_embedding_b = net.encoder(test_data_b)
    test_embedding_c = net.encoder(test_data_c)
    test_embedding_d = net.encoder(test_data_d)
    test_embedding = torch.concat([test_embedding_a, test_embedding_b, test_embedding_c, test_embedding_d], dim=-1)
    test_embedding = test_embedding /torch.norm(test_embedding, dim=1)[..., None]
    torch.save(test_embedding, f'embedding/test_embedding_{seed}')

    val_embedding_a = net.encoder(val_data_a)
    val_embedding_b = net.encoder(val_data_b)
    val_embedding_c = net.encoder(val_data_c)
    val_embedding_d = net.encoder(val_data_d)
    val_embedding = torch.concat([val_embedding_a, val_embedding_b, val_embedding_c, val_embedding_d], dim=-1)
    val_embedding = val_embedding / torch.norm(val_embedding, dim=1)[..., None]
    torch.save(val_embedding, f'embedding/val_embedding_{seed}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--width_in', type=int, default=32,
