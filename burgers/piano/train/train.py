...
def train(config, net, timestring):
    ...
    pre_training = str(config.seed)

    train_data = torch.load(data_path+'data_train').to(device).float()[:, ::2, :]
    train_embedding = torch.load(embedding_path+'train_embedding_'+pre_training).to(device)
    num_train = train_data.shape[0]

    test_data = torch.load(data_path+'data_test').to(device).float()[:, ::2, :]
    test_embedding = torch.load(embedding_path+'test_embedding_'+pre_training).to(device)
    num_test = test_data.shape[0]

    val_data = torch.load(data_path+'data_val').to(device).float()[:, ::2, :]
    val_embedding = torch.load(embedding_path+'val_embedding_'+pre_training).to(device)
    ...
    for step in range(config.num_iterations+1):
        ...
        for i in range(int(num_train//batch_size)):
            input_data, output_data = torch.zeros(batch_size, size, w).to(device), torch.zeros(batch_size, size, w).to(device)
